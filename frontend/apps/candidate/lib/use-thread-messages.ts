 ...
 *  defaults differ. Receive is a 5s poll (paused on a hidden tab); send is optimistic with
 *  rollback; mark-read fires on open / on new inbound, guarded against per-poll-tick loops. */
export function useThreadMessages(applicationId: string, side: SenderSide) {
  ...
  // Receive: 5s open cadence, paused on a hidden tab (liveness vs idle-cost).
  const q = useQuery({
