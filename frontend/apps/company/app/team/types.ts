// Typed contract for TeamService. The proto/servicer aren't generated yet, so the roster,
// invite dialog, and permission grid code against this shape until `pnpm gen` exposes
// `api.team.*`. camelCase mirrors protobuf-es (lastActiveAt / invitedBy / userId / pageSize).
...
// The seam the roster codes against: the real client (createTeamClient) and the mock
// (makeMockTeamClient) both satisfy it, so component code never changes at integration.
