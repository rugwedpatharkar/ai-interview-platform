...
/** Bell trigger + unread Badge + feed DropdownMenu — the company twin of the candidate bell, with
 *  a COMPANY_KINDS render-filter so a recruiter never sees candidate-only/practice rows. Owns the
 *  badge poll, the lazy feed, and mark-read / mark-all-read. Mounted in the company shell. */
export function NotificationBell() {
  ...
  // Belt-and-suspenders: drop any non-recruiter kind before rendering (backend already excludes).
  const items = filterCompanyKinds(feed.data?.notifications ?? []);
