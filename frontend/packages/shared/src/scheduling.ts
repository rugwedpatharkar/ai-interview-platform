...
//
// Shared scaffold (lifted from candidate + company). Role-specific deltas stay in each app's
// lib/scheduling.ts: candidate's `candidateListQueryKey`, company's `MAX_SLOTS`, and the
// `cancelledBy` value (passed into `schedulingClient`/`makeMockSchedulingClient`).
...
// Query key owned here so views + invalidation never drift.
...
export const SCHEDULING_USE_MOCK = process.env.NEXT_PUBLIC_MOCK === "1";
 ...
 * `booked`; `cancel` flips to `cancelled` (attributing `cancelledBy` to the calling role). Keyed
 * by application id so a recruiter `proposeSlots` and a candidate `chooseSlot` against the same
 * id share one store within a session. */
export function makeMockSchedulingClient(cancelledBy: string): SchedulingClient {
  ...
  return {
    ...
    cancel: async (applicationId) => {
      const dto = seed(applicationId);
      const next: ScheduleDTO = { ...dto, status: "cancelled", cancelledBy };
      store.set(applicationId, next);
      return structuredClone(next);
    },
 ...
 * `cancelledBy` attributes a mock cancellation to the calling role. Memoize the result at the
 * call site (`useMemo`) so the mock's store survives re-renders. */
export function schedulingClient(api: unknown, cancelledBy: string): SchedulingClient {
  return SCHEDULING_USE_MOCK ? makeMockSchedulingClient(cancelledBy) : createSchedulingClient(api);
}
