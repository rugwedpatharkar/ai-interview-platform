...
async def fan_out_match(payload, *, jobs, publisher, limit):
    ...
    if not user_id:
        # Malformed profile.parsed: raise so the consumer dead-letters it (the caller
        # must not swallow this — a silent ack would lose the candidate's fan-out).
    ...
    if len(open_jobs) >= limit:
        log.warning("recommend fan-out capped at {} jobs for user {}", limit, user_id)
